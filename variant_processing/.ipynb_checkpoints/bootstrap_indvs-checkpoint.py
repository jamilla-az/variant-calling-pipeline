...
def main():
    parser = argparse.ArgumentParser(description='Bootstrap estimator for number of segregating sites')
    
    parser.add_argument('base_file', type=str, help='table of base (CA) individuals labels')
    parser.add_argument('indv_file', type=str, help='table of X pop individuals labels')
    parser.add_argument('output_label', type=str, help='pop label for output file')
    parser.add_argument('var_type', type=str, help='variant type label for output file: all, genic, exon, nonsyn')
    ...
    base_df = pd.read_csv(args.base_file, sep = '\t', header=None, index_col = 0, names=['loc','line','num'])
    
    #dataframe of individuals to use from X pop
    indv_df = pd.read_csv(args.indv_file, sep = '\t', header=None, index_col = 0, names=['loc','line','num'])
    
    #dataframe of genotypes x individuals
    geno_df = pd.read_csv(f'output_files/genotype_matrix_{args.var_type}.table', sep='\t')
    #print(geno_df.head())
    ...
    for n in np.arange(0,100):
        ...
        lines = base_df['line'].loc[ids].value_counts().to_frame(name='num_indv')
        ...
        for k in np.arange(0,len(lines)):
            inds = random.choices(indv_df.loc[indv_df['line']==l[k]].index, k=lines['num_indv'].values[k])
        ...
        for i in np.arange(0, len(geno_counts)):
            hom_ref = geno_counts['hom-ref'].iloc[i] # num ref alleles = num hom ref genotypes
            hom_alt = geno_counts['hom-alt'].iloc[i] # num alt alleles = num hom alt genotypes
            het_sample = np.random.choice(2, geno_counts['het'].iloc[i]) #random ref/alt sampling for hets
    ...
    print(f'saving list as output_files/boot_{args.output_label}_segsites.table')
    pd.DataFrame({'seg_sites':seg_sites}).to_csv(f'output_files/boot_{args.output_label}_segsites_{args.var_type}.table', sep='\t', index = False)
