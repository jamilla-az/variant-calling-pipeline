...
def main():
    ...
    geno_df = pd.read_csv('output_files/genotype_matrix.table', sep='\t')
    #print(geno_df.head())
    ...
    print(f'saving list as output_files/boot_{args.output_label}_segsites_v2.table')
    pd.DataFrame({'seg_sites':seg_sites}).to_csv(f'output_files/boot_{args.output_label}_segsites_v2.table', sep='\t', index = False)
